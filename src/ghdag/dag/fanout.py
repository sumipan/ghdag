...
def parse_fanout_spec(result_path: str | None) -> FanOutSpec | None:
    ...
    parses everything from that line as YAML. Returns None if no block is found
    or parsing fails (with a warning log). Raises ValueError if child ``id``
