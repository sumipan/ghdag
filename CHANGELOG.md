...
- `DagEngine` SIGTERM handler is now two-stage: first SIGTERM enters drain mode (`_draining = True`, calls `mark_interrupted_all`, sets `_drain_deadline = now + task_timeout`); a second SIGTERM while draining sets `_shutdown = True` for immediate exit. `task_timeout is None` skips the deadline and drains indefinitely (sumipan/nexus#3666).
...
- `TaskLauncher.adopt_orphans()`: running files with `interrupted_at` are now treated as interrupted restarts instead of plain orphans. Live processes are killed (SIGTERM + kill_grace + SIGKILL); the running file is consumed and the task is queued for same-uuid rerun via `_pending_reruns`, up to `_MAX_INTERRUPTED_RERUNS = 1`. Tasks at the rerun limit fall back to `DONE_ORPHANED_ON_RESTART` (sumipan/nexus#3666).
...
- `QuotaGate` に `drain/resume/begin_run/finish_run/wait_idle` を追加し、draining と running registry を公開 API として扱えるようにした
...
- `TaskLauncher` の launch admission を `begin_run()` へ置換し、`Popen` 失敗時の running reservation rollback と subprocess 終了直後の `finish_run()` を保証した
- `ghdag quota drain|resume|status` を拡張し、engine ごとの `queued/deferred/running/idle` と `unresolved` を JSON 集計できるようにした
