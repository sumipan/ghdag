"""Tests for create_github_client / create_github_clients factory functions."""
...
def test_create_github_client_returns_github_client():
...
def test_create_github_clients_returns_one_per_repo(monkeypatch):
    ...
    assert all(isinstance(c, GitHubClient) for c in clients)
...
def test_create_github_clients_uses_env_token(monkeypatch):
    ...
    assert clients[0]._token == "g-token"
