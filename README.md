...
For cursor stream-json, the result file and `call_text` body prefer the last **complete** assistant message after the final `tool_call` (non-empty top-level `model_call_id`, or text that exactly equals the concatenation of preceding partial assistant fragments). If no such candidate exists, adapters fall back to the last `type=result` payload, a single JSON object, or raw stdout. Events JSONL is unchanged and still keeps intermediate narration.
